...
def contrastive_loss(embedding_1, embedding_2, margin=1.0):
    """
    计算对比损失：正样本对和负样本对
    embedding_1, embedding_2: 两个输入的潜在空间表示
    margin: 对比损失的边际
    """
    # 计算余弦相似度
    cosine_similarity = F.cosine_similarity(embedding_1, embedding_2)

    # 对比损失：正样本对的损失和负样本对的损失
    # 正样本对：余弦相似度越大越好
    # 负样本对：余弦相似度越小越好
    positive_loss = torch.mean(1 - cosine_similarity)  # 正样本损失，越相似越小
    negative_loss = torch.mean(torch.clamp(margin - cosine_similarity, min=0))  # 负样本损失，越不相似越大
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, z_i: torch.Tensor, z_j: torch.Tensor) -> torch.Tensor:
        """
        计算对比损失
        z_i, z_j：来自两种不同增强方式的样本表示
        """
        sim_ij = F.cosine_similarity(z_i, z_j, dim=-1)  # 正样本相似性
        loss = -torch.log(torch.exp(sim_ij / self.temperature) /
                          (torch.exp(sim_ij / self.temperature) + torch.exp(-sim_ij / self.temperature)))
        return loss.mean()


def train_AE_model(net, data_loaders={}, optimizer=None, loss_function=None, n_epochs=100, scheduler=None, load=False,
                   save_path="model.pkl"):
    if (load != False):
...
#
def train_DAE_model(net, data_loaders={}, optimizer=None, loss_function=None, n_epochs=100, scheduler=None, load=False,
                    save_path="model.pkl"):
    if (load != False):
        if (os.path.exists(save_path)):
            net.load_state_dict(torch.load(save_path))
    ...
    for epoch in range(n_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for batchidx, (x, _) in enumerate(data_loaders[phase]):
                ...
                #
                # # 获取潜在表示
                # z_i = net.encoder(z)  # 噪声输入的潜在表示
                # z_j = net.encoder(x)  # 原始输入的潜在表示
                #
                # contrastive_loss_fn = ContrastiveLoss(temperature=0.07)
                #
                # # 然后可以在训练循环中调用它来计算损失
                # contrastive_loss_value = contrastive_loss_fn(z_i, z_j)
                #
                # # 总损失 = 重建损失 + 对比损失
...
def train_VAE_model(net, data_loaders={}, optimizer=None, n_epochs=100, scheduler=None, load=False,
                    save_path="model.pkl", best_model_cache="drive"):
    if (load != False):
        if (os.path.exists(save_path)):
            net.load_state_dict(torch.load(save_path))
            return net, 0
    ...
    for epoch in range(n_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for batchidx, (x, _) in enumerate(data_loaders[phase]):
                ...
                loss = vae_loss(output[0], output[1], output[2], output[3], recon_loss,
                                data_loaders[phase].batch_size / dataset_sizes[phase])
...
def train_CVAE_model(net, data_loaders={}, optimizer=None, n_epochs=100, scheduler=None, load=False,
                     save_path="model.pkl", best_model_cache="drive"):
    if (load != False):
        if (os.path.exists(save_path)):
            net.load_state_dict(torch.load(save_path))
...
def train_predictor_model(net, data_loaders, optimizer, loss_function, n_epochs, scheduler, load=False,
                          save_path="model.pkl"):
    if (load != False):
        if (os.path.exists(save_path)):
            net.load_state_dict(torch.load(save_path))
...
def train_DaNN_model2(net, source_loader, target_loader,
                      optimizer, loss_function, n_epochs, scheduler, dist_loss, weight=0.25, GAMMA=1000,
                      epoch_tail=0.90,
        ...
        for phase in ['train', 'val']:
            ...
            for batchidx, (x_src, y_src) in enumerate(source_loader[phase]):
                ...
                if encoderrep.shape[0] < k:
                    ...
                    edgeList = calculateKNNgraphDistanceMatrix(encoderrep.cpu().detach().numpy(),
                                                               distanceType='euclidean', k=10)
