...
from models import (AEBase, PretrainedPredictor, PretrainedVAEPredictor, VAEBase, FocalLoss)
import matplotlib
import random
...
def run_main(args):
    ...
    para = str(args.bulk) + "_data_" + str(args.data_name) + "_drug_" + str(args.drug) + "_bottle_" + str(
        args.bottleneck) + "_edim_" + str(args.encoder_h_dims) + "_pdim_" + str(
        args.predictor_h_dims) + "_model_" + reduce_model + "_dropout_" + str(args.dropout) + "_gene_" + str(
        args.printgene) + "_lr_" + str(args.lr) + "_mod_" + str(args.mod) + "_sam_" + str(args.sampling)  # (para)
    ...
    # Read data
    import pandas as pd
    data_r = pd.read_csv(data_path, index_col=0)  # RNA-Seq表达数据文件
    label_r = pd.read_csv(label_path, index_col=0)  # 药物敏感性标签数据文件
    if args.bulk == 'old':
        # 选择旧数据库的部分数据。在这种情况下，data_r和label_r会分别截取前805行的数据。
        data_r = data_r[0:805]
        label_r = label_r[0:805]
    elif args.bulk == 'new':
        # 选择新数据库的部分数据。在这种情况下，data_r和label_r会截取从第806行到文件末尾的数据。
        data_r = data_r[805:data_r.shape[0]]
        label_r = label_r[805:label_r.shape[0]]
    else:
        print("two databases combine")
    label_r = label_r.fillna(na)  # 用na参数指定的值来填充label_r中的缺失值。这是为了确保标签数据的完整性。
    ut.save_arguments(args, now)  # 调用ut模块中的save_arguments函数，将命令行参数args和时间戳now保存起来，可能是为了记录实验参数的设置。
    ...
    # 首先根据指定的药物敏感性标签数据中的缺失值和用户输入的高度可变基因选择阈值 g_disperson 进行数据预处理。
    # 如果 g_disperson 不为 None，则会使用 ut.highly_variable_genes 函数选择高度可变基因，并将数据的列名称更改为选定的高度可变基因。
    ...
    # Do PCA if PCA_dim!=0
    # PCA降维 代码将对数据进行主成分分析（PCA）降维操作，将数据降低到指定的维度。
    if PCA_dim != 0:
    ...
    # Scaling data 数据标准化
    # 对RNA-Seq数据进行最小-最大缩放，将数据的值缩放到指定的范围内。
    mmscaler = preprocessing.MinMaxScaler()

    data = mmscaler.fit_transform(data)
    label = label.values.reshape(-1, 1)

    # 标签编码： 使用LabelEncoder对标签数据进行编码，将药物名称转换为整数标签。这是因为深度学习模型通常要求标签为整数值。
    le = LabelEncoder()
    ...
    # Defined the model of predictor 这些模型都包括编码器和预测器部分，其结构和参数在模型的构造函数中定义。
    if reduce_model == "AE":  # （自动编码器）
    ...
    if reduce_model == "DAE":  # （去噪自动编码器）
        model = PretrainedPredictor(input_dim=X_train.shape[1], latent_dim=dim_au_out, h_dims=encoder_hdims,
                                    hidden_dims_predictor=preditor_hdims, output_dim=dim_model_out,
                                    pretrained_weights=bulk_encoder, freezed=bool(args.freeze_pretrain),
                                    drop_out=args.dropout, drop_out_predictor=args.dropout)
    elif reduce_model == "VAE":  # （变分自动编码器）
        model = PretrainedVAEPredictor(input_dim=X_train.shape[1], latent_dim=dim_au_out, h_dims=encoder_hdims,
                                       hidden_dims_predictor=preditor_hdims, output_dim=dim_model_out,
                                       pretrained_weights=bulk_encoder, freezed=bool(args.freeze_pretrain),
                                       z_reparam=bool(args.VAErepram), drop_out=args.dropout,
    ...
    model, report = t.train_predictor_model(model, dataloaders_train,
                                            optimizer, loss_function, epochs, exp_lr_scheduler, load=load,
    ...
    if (args.printgene == 'T'):
        ...
        target = 1
        attr, delta = ig.attribute(bulk_X_allTensor, target=1, return_convergence_delta=True,
    ...
    classes = report_df.index[:-3]  # 最后三行是 'accuracy', 'macro avg', 'weighted avg'，我们不需要
    ...
    for i, class_name in enumerate(classes):
        TP = conf_matrix[i, i]  # 对角线上的元素为 TP
        FP = conf_matrix[:, i].sum() - TP  # 列的和减去 TP 为 FP
        FN = conf_matrix[i, :].sum() - TP  # 行的和减去 TP 为 FN
        TN = conf_matrix.sum() - (TP + FP + FN)  # 总和减去 TP, FP 和 FN 为 TN

        # 将计算结果添加到 DataFrame 中
        report_df.loc[class_name, 'TP'] = TP
        report_df.loc[class_name, 'TN'] = TN
        report_df.loc[class_name, 'FP'] = FP
        report_df.loc[class_name, 'FN'] = FN

        # 打印每个类别的 TP, TN, FP, FN
        print(f"Class {class_name}: TP = {TP}, TN = {TN}, FP = {FP}, FN = {FN}")

    # 将结果保存到 CSV 文件中
    report_df.to_csv("save/result/" + reduce_model + select_drug + now + '_report.csv')

    # 打印整个报告（包括 TP, TN, FP, FN）
    print("\nFull Classification Report with TP, TN, FP, FN:")
    print(report_df)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result', type=str, default='save/results/result_',
    ...
    parser.add_argument('--PCA_dim', type=int, default=0,
                        help='Number of components of PCA  reduction before training. If 0, no PCA will be performed. Default: 0')
    ...
    parser.add_argument('--bulk_encoder', '-e', type=str, default='save/bulk_encoder/',
                        help='Path of the pre-trained encoder in the bulk level')
    parser.add_argument('--pretrain', type=str, default="True",
                        help='Whether to perform pre-training of the encoder,str. False: do not pretraing, True: pretrain. Default: True')
    ...
    parser.add_argument('--data_name', type=str, default="GSE110894",
    ...
    parser.add_argument('--bulk_model', '-p', type=str, default='save/bulk_pre/',
                        help='Path of the trained prediction model in the bulk level')
