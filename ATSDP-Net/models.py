...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # inputs: 模型的logits输出，形状为 (batch_size, num_classes)
        # targets: 实际标签，形状为 (batch_size,)

        # 计算交叉熵
        BCE_loss = F.cross_entropy(inputs, targets, reduction='none')  # 不进行平均
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        # x 的形状是 [seq_len, input_dim]
        # 添加一个批处理维度，使其成为 [seq_len, batch_size, input_dim]
        x = x.unsqueeze(1)  # x 的形状变为 [200, 1, 512]

        # 应用多头注意力机制
        # 在自注意力场景中，Q, K, V 是相同的
        attn_output, _ = self.attention(x, x, x)

        # 移除批处理维度，使其回到原始形状 [seq_len, input_dim]
...
class Predictor(nn.Module):
    def __init__(self,
        ...
        self.simple_mha = MultiHeadAttention(input_dim=o_dim, num_heads=8)
